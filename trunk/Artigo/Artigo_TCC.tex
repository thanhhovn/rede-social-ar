...
\indent \indent
A plataforma Android tem como base o SO Linux, com isso herdou algumas das características desse SO, como o, gerenciamento de \textit{drivers}, segurança, memória, ser multitarefa (permite que mais de uma aplicação possa ser executada ao mesmo tempo) e de código aberto.

Possui uma máquina virtual (\textit{Dalvik}) própria baseada na máquina virtual JAVA, mas otimizada para consumir pouca memória e poder usar mais de uma instância simultaneamente sem prejudicar o desempenho do dispositivo. Além disso, os arquivos executáveis possuem extensão .dex, que é uma versão melhorada do .class para dispositivos móveis. Usa a linguagem JAVA como linguagem de programação.
...
O Android está em constante evolução. Atualmente, o Android está na versão 4.0, conhecida como Ice Cream Sandwich. A Cada nova versão ou plataforma, são acrescentadas novas APIs (funcionalidades), possibilitando ao desenvolvedor tirar o máximo dos recursos do dispositivo móvel e com isso, possibilitar ao desenvolvedor a criar aplicações robustas.
...
É fácil perceber, através da imagem \ref{fig_cicloVidaAtivity}, quais são as fases do ciclo onde o SO pode liberar recursos, ou seja, finalizar alguma \textit{Activity} que está muito tempo sem ser executada. E que também, no ciclo da \textit{Activity}, os únicos métodos que não são executados mais de uma vez são os métodos onCreat e o onDestroy.
	...
	\indent \indent
	"A classe View é responsável por desenhar algo na tela e tratar seus eventos se necessários, e é a classe-base para todos os componentes visuais Android" \cite{Lecheta2010}. Aqui será mostrado alguns desses componentes visuais, tais como, TextView, EditText, Button, CheckBox,  RadioButton e RadioGroup. No Android, todos os elementos da aplicação são representados por tags. E toda tag deve ser aberta e fechada, representada respectivamente por \flq nome\frq \ e \flq /nome\frq. Na figura xx, é possível ver um exemplo do uso desses componentes.
	...
	O componente EditText, é uma especialização do componente TextView. E representa um campo texto, onde o usuário pode digitar. Esse componente é referenciado no arquivo pela tag \flq EditText\frq. Veja um exemplo no código \ref{EditText}.
...
No código \ref{fig_exAndroidManifest} é mostrado um exemplo de Arquivo AndroidManifest.xml, onde é pedido a permissão para a aplicação ter acesso a internet.
