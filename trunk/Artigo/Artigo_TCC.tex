...
\indent\indent 
Mais um sonho se realiza, dentre muitos que haverei de realizar. Finalmente cheguei ao objetivo de tantas horas de estudo e dedicação: a formatura.

Sou muito grato a Deus, sempre maravilhoso, por mais esta conquista, por me dar força e ânimo para concluir esta caminhada. O Senhor sempre foi meu refúgio e fortaleza! As lutas foram muitas, mas com sua grandeza, amor e poder me levantou e fez com que este sonho se realizasse.
 
Agradeço a minha mãe pelo amor e apoio incondicional, que nunca mediu esforços para que eu chegasse a concluir este sonho e ao meu irmão, por fazer parte dessa trajetória.

Sou grato a minha namorada Isleide pelo apoio, amizade e compreensão. Saiba que você faz parte desse sonho.

Á minha tia Maria de Lourdes que me incentivou a continuar nesse caminho. Ao meu primo Adriano (Neto) que sempre me recebeu com carinho em sua casa naqueles momentos em que me encontrava em dificuldade para realizar minhas tarefas e também aos demais familiares.
 
Aos meus amigos e aos inesquecíveis colegas de turma pela força para que eu chegasse até aqui.
 
Ao professor Andrés Menéndez pela orientação, amizade e principalmente, pela paciência, sem a qual este trabalho não se realizaria.
 
Aos professores do Departamento de Sistemas de Informação pelos seus ensinamentos. Em especial ao professor André Luís pelo incentivo, amizade e pelos conselhos que ajudaram na formação do meu caráter profissional.
 
Aos funcionários da UFS, que durante esses anos contribuíram de algum modo para o meu enriquecimento pessoal e profissional.
 
Enfim, obrigado a todos que torceram por mim!
\newpage
%---------------------- EPÍGRAFE I (OPCIONAL)--------------
\begin{epigrafe}
...
\section{Introdução}
	...
	\end{enumerate}
  	 ...
  	 \indent Entretanto, somente na década de 80 que surgiu o primeiro projeto de realidade aumentada. Este projeto foi desenvolvido pela Força Aérea Americana e tratava-se de um simulador de cockpit de avião, com visão ótica direta, misturando elementos virtuais com o ambiente físico do usuário ~\cite{MarcosWagnerS.Ribeiro2011}.
  	 
\section{Conceitos e Definições}  	 
	\indent \indent 
	 A RA é caracterizada pela inserção de objetos virtuais no ambiente físico, mostrado ao usuário, em tempo real, com o apoio de algum dispositivo tecnológico, usando a interface do ambiente real, adaptada para visualizar e manipular os objetos reais e virtuais  ~\cite{Tori2006a}. 
...
novas maneiras de interagir com o mundo físico e permitindo o acesso a dados extremamente valiosos de uma maneira natural". -David Helgason, CEO da Unity Technologies. %dúvida
	...
	\end{figure}
	 ...
	 \indent
	 A RA é utilizada em muitas soluções desde jogos a mídia e marketing interativo, passando por instruções e módulo de ajuda, a realidade aumentada abre espaço para novos aplicativos e serviços móveis. Indo mais fundo, essa tecnologia deverá ter grande impacto no relacionamento das pessoas, através de novas maneiras de realizar visualização, comunicação, obtenção da informação e interação com pessoas.  
...
\section{Tipos de Sistemas de Realidade Aumentada}
	...
	O sistema de visão ótica direta necessita do uso de óculos ou capacete, com lentes que permitam o recebimento direto da imagem real, ao mesmo tempo em que permitam a projeção de imagens virtuais devidamente ajustadas com a cena real nos olhos do usuário. Veja um exemplo desse sistema na figura ~\ref{fig_otico}. 
	\begin{figure}[h]
		\begin{center}			
...
\section{LOOK}
	\indent \indent	
	LOOK é um framework desenvolvido por um grupo de estudantes da Universidade de Computação de Madrid, no ano de 2011, e tem como objetivo dar suporte ao desenvolvimento de aplicações que fazem uso da Realidade Aumentada. Look foi desenvolvido na plataforma Java e até a data da finalização (20/04/2013) desse TCC (trabalho de conclusão de curso) ainda se encontrava em versão beta.

	Look está dividido em quatro módulos independentes [4], podendo o desenvolvedor escolher aquele(s) que melhor atenda(m) as suas necessidades, são eles:
	\begin{itemize}
	 	\item Localização em interiores;
 	 	\item Representação gráfica;
	 	\item Interações com os elementos virtuais;
	 	\item Acesso a serviços remotos.
	\end{itemize}
	 
	Para possibilitar a localização em interiores o Look faz uso de redes Wifi e Bluetooth, permitindo que objetos sejam localizados onde o GPS não oferece uma precisão considerável ou não há sinal disponível. Porém, seu nível de precisão é restrito a locais pequenos e necessita de uma série de etapas para seu funcionamento, dentre eles está o fornecimento da planta baixa do edifício.
	 
Um fator importante na hora de criar uma aplicação de realidade aumentada é a forma com que os elementos virtuais serão apresentados para o usuário. Pensando nesse fator de importância: aparência agradável, o Look permite criar aplicações visualmente amigáveis através do uso de elementos no formato 2D e 3D, onde esses elementos podem ter formas básicas associadas tais como: estilo, cor e textura, como também elementos gráficos sendo eles pontos, vetores e matrizes. E ainda, oferece suporte a componentes visuais da plataforma Android. Além disso, é possível que o usuário interaja com os elementos virtuais, onde o usuário poderá tocar, arrastar, soltar, além de permitir focá-los através de eventos gerados pelos próprios componentes ou até mesmo pela câmera, além de permitir ter uma ação associada a cada um deles. Já módulo de acesso a serviços remotos tem o intuito de facilitar o acesso a serviços externos, por exemplo: banco de dados que está localizado em algum servidor remoto.
	 
Conforme foi mencionado, o \textit{framework Look} implementa 4 módulos independentes que facilitam o desenvolvimento de aplicações de realidade aumentada. Contudo, para o desenvolvimento deste trabalho só foram utilizados os módulos de representação gráfica e interações com os elementos virtuais. O módulo de localização de interiores não foi utilizado, porque não faz uso do GPS para obter as coordenadas geográficas e o módulo de acesso a serviços remotos não foi utilizado porque, para esse fim, foi desenvolvido um Web Service próprio que será acessado pela aplicação Android através da biblioteca ksoap2 [3].

		
Segundo [4] o \textit{framework Look} funciona corretamente nos seguintes dispositivos:
	\begin{itemize}
	 \item \textit{Google Nexus One};
 	 \item \textit{HTC Desire HD};
	 \item \textit{HTC Tattoo}.
	\end{itemize}
		 	
		\subsection{Requisitos Não Funcionais}
		\indent \indent
		O \textit{framework Look} possui algumas condições [4] para o seu uso, são elas:
			\begin{itemize}
	 		\item Interação com o usuário por meio de interfaces táteis;
 	 		\item Funcionamento em dispositivos Android na versão 2.2 ou superior;
	 		\item Não necessita de hardware adicional;
	 		\item A persistência deverá ser realizada em base de dados;
	 		\item O dispositivo deve possuir câmera, wifi ou bluetooth.
				
		\subsection{Arquitetura}
		\indent \indent
	  Para ficar mais claro, veremos como está divida a arquitetura geral do \textit{framework Look} na figura ~\ref{fig_ArquiteturaLook}.		
		\begin{figure}[h]
		\begin{center}
			\includegraphics{Figuras/Arquitetura_Look.jpg} 
			\caption{Arquitetura do \textit{Framework Look}.}
			\label{fig_ArquiteturaLook}
		\end{center}
	\end{figure}
		
Para este projeto a única camada que esta sendo utilizada é a camada de realidade aumentada. Essa camada contém elementos com capacidade de representação gráfica que serão utilizadas para a criação da interface de interação com o usuário. Dentre esses elementos temos a câmera, visualização 3D, visualização 2D e HUD, onde cada um possui características próprias e funcionalidades específicas. A seguir temos uma descrição do que cada elemento oferece para a aplicação:
			\begin{itemize}
	 		\item Câmera - Módulo responsável por permitir o uso da câmera, interação com os elementos virtuais através de eventos gerados pela mesma e a sobreposição dos elementos virtuais sobre a imagem capturada;
 	 		\item Módulo que possui todos os recursos necessários para visualizar os elementos virtuais em três dimensões;
	 		\item Visualização 2D - Módulo que possui todos os recursos necessários para visualizar os elementos virtuais em duas dimensões;
	 		\item HUD - Módulo que serve para agregar novos elementos a aplicação, onde podemos incluir botões, views, ou seja, os componentes que estamos acostumados a trabalhar quando desenvolvemos aplicações para android.
	 		\end{itemize}
	 		
No capítulo aplicação iremos conhecer melhor cada elemento mencionado a respeito da camada de realidade aumentada.
...
O Layar oferece vários tipos de experiências aos usuários, com características interativas e imersivas, como objetos em 3D e animação. Algumas das experiências oferecidas pelo Layar são: jogar games usando como cenário o próprio ambiente (veja na figura ~\ref{fig_jogo}), encontrar locais próximos, incluindo cafés, lojas e outros negócios, bem como locais históricos, monumentos e o reconhecimento de objetos do mundo real mostrando experiências digitais em cima deles permitindo que os usuários interajam com esses objetos, que podem ser encontrados em revistas, jornais, cardápios de restaurantes, cartazes publicitários e livros. Veja um exemplo de visão computacional no uso de reconhecimento de objetos na figura ~\ref{fig_reconhecimentoObjeto}.
	...
	\indent \indent
		Este projeto faz parte de um projeto maior, Wikinarua. O Wikinarua visa à criação de uma rede social como proposta de inclusão artística. O software RUA assim como o Wikinarua foram desenvolvidos na Universidade de Brasília. O RUA Permite o compartilhamento de fotos, vídeos, músicas, comunicação entre os usuários, a visualização de um monumento histórico e suas ruínas em diferentes momentos de sua existência, observar os mínimos detalhes de algumas obras de arte na tela do dispositivo e o reconhecimento de imagens para a identificação de diversos espaços e pontos importantes de cidades de todo o Brasil. 
...
%% Fim de RA
\chapter{GeoProcessamento}
\label{geoProcessamento}
	\indent \indent
	A obtenção de informações sobre a distribuição geográfica de fenômenos e objetos é parte importante para a organização da sociedade. Antes estas informações estavam contidas somente em mapas e documentos em papel impresso, mas a partir da segunda metade do século XX foi possível armazenar e representar tais informações em ambiente computacional, culminando no advento da prática do Geoprocessamento, tido como: 
	\begin{citacao}
...
O Geoprocessamento engloba diversas tecnologias, dentre essas tecnologias, se destacam: o sensoriamento remoto, a digitalização de dados, a automação de tarefas cartográficas, a utilização de Sistemas de Posicionamento Global (GPS) e os Sistemas de Informações Geográficas (SIG). O geoprocessamento dos dados obtidos por esse conjunto de tecnologias tem como objetivo a geração de informações espaciais (mapas, tabelas, relatórios, estatísticas, gráficos etc.), para auxiliar na tomada de decisão.

Como vimos anteriormente, o geoprocessamento é uma área complexa que envolve diversas tecnologias. Por esse motivo, serão abordados, somente, assuntos relevantes para a conclusão desse trabalho. 

\section{SIG (\textit{Geographic Information System})}
\indent \indent
Segundo [6], GIS é um sistema de computador composto de hardware, software, dados e procedimentos, construído para permitir a captura, gerenciamento, análise, manipulação, modelamento e exibição de dados referenciados geograficamente para solucionar, planejar e gerenciar problemas. Os dados que são manipulados pelo SIG são provindos de banco de dados geográficos, local onde são armazenados os dados georreferenciados.

Para ter uma noção do poder e importância do GIS imagine o seguinte senário, o dono de uma distribuidora de bebidas esta querendo distribuir refrigerantes em todos os pontos de revenda da cidade, utilizando-se do menor número possível de veículos. Considerar o trajeto: fluxo do tráfego, distâncias percorridas, quantidade de pontos visitados num intervalo X de tempo, quantidade média entregue em cada ponto, etc. Um tipo de problema como esse não poderia ser solucionado através de um banco de dados convencional, pois estes não armazenam o componente espacial, nem relações de topologia. Para esse caso apenas um BDG (Banco de dados geográfico ou espacial) permitiria que essas questões fossem respondidas com base na posição geográfica de cada elemento do banco.

Denota-se por topologia a estrutura de relacionamentos espaciais, como vizinhança, proximidade e pertinência que podem se estabelecer entre objetos geográficos. O armazenamento da topologia permite a realização de consultas a um banco de dados espacial, que não seriam possíveis de outra maneira.

Os SGBD convencionais não suportam a implementação de BDG de forma nativa. Por esse motivo, diversas empresas desenvolvedoras desses programas criaram extensões espaciais que possibilitam trabalhar com esse tipo de informação espacial. Um exemplo do uso dessas extensões é o PostGis, que é a extensão espacial do SGBD PostgreSQL. A figura ~\ref{fig_sig} mostra a relação entre o BDG, o PostgreSQL e o PostGis.

	\begin{figure}[h]
		\begin{center}
			\includegraphics{Figuras/PostGis_SGBD_PostgreSQL.png} 
			\caption{Relação entre o BDG, o PostgreSQL e o PostGis}
			\label{fig_sig}
		\end{center}
	\end{figure}	
	Quando integramos uma extensão espacial a um banco relacional obtemos elementos com propriedades espaciais. É possível ver alguns desses elementos na tabela \ref{tab:propriedadesEspeciais}.
	\begin{table}[htb!]
	\begin{center}
	{\footnotesize
	\begin{tabular}{|p{5cm}|p{5cm}|}\hline 
		Geometria Matricial & Uma matriz de elementos em um subconjunto real \\ 
		\hline 
		Geometria Vetorial & Pontos, linhas ou poligonos \\ 
		\hline 
		Atributo Espacial & Atributo de um objeto que tem como domínio um conjunto de geometrias \\ 
		\hline 
		Objeto Espacial & Objetos com atributos espaciais \\ 
		\hline 
		Componente Espacial (ou Geometria de um Objeto) & Valor de um atributo espacial \\ 
		\hline 
		Banco de Dados Espacial & Banco de dados que funciona também com objetos espaciais \\ 
		\hline 
		Consulta Espacial & Consultas em um banco de dados espacial \\ 
		\hline 
	\end{tabular} 
	\end{center}
	\caption{Tabela com algumas propriedades espaciais e suas respectivas descrições.}
	\label{tab:propriedadesEspeciais}
	\end{table}
	
	\subsection{Dados espaciais}
\indent \indent
São dados que possuem uma localização geográfica definida, utilizados para representar o espaço através de pontos, polígonos, linhas e imagens. O espaço pode ser bidimensional (2D) ou tridimensional (3D). 
Para este trabalho foi utilizado somente o espaço bidimensional. Logo, os pontos são representados por coordenadas (x, y). Essa representação atende as necessidades desse projeto, porque para localizar um usuário ou uma marcação basta sabermos somente sua latitude e longitude.

Como vimos, um ponto é representado por um par de coordenadas (x, y), no entanto as linhas são formadas por dois ou mais pares de coordenadas (x, y) e os polígonos são formados por um conjunto de linhas que se encontrando formando a área do polígono. A figura ~\ref{fig_dadosEspaciais} apresenta alguns exemplos desses dados.
 	\begin{figure}[h]
		\begin{center}
			\includegraphics{Figuras/estrutura_de_dados_espaciais.png} 
			\caption{Tipos de dados espaciais}
			\label{fig_dadosEspaciais}
		\end{center}
	\end{figure}
Para a manipulação desses dados temos um conjunto de operadores espaciais. Sendo que os mesmos foram formalizados por [8]. A figura ~\ref{fig_operadoresEspaciais} mostra alguns desses operadores. 
 	\begin{figure}[h]
		\begin{center}
			\includegraphics{Figuras/operadores_de_dados_espaciais.png} 
			\caption{Operadores espaciais}
			\label{fig_operadoresEspaciais}
		\end{center}
	\end{figure}
	Segue uma breve explicação sobre as operações mostrada na figura ~\ref{fig_operadoresEspaciais}:
	\begin{itemize}
	 		\item Igual: Verifica se duas geometrias são iguais;
 	 		\item Distância: Calcula a distância espacial entre duas geometrias;
	 		\item Intercepta: Verifica se duas geometrias se interceptam ou se cruzam;
	 		\item União: Retorna o resultado da união entre duas geometrias;
	 		\item Área: Calcula a área de uma determinada geometria.
	 		\end{itemize}
	Com o uso desses operadores será possível, por exemplo, encontrar todos os usuários próximos ou marcações de acordo com sua localização, distância entre dois usuários, saber o local específico que um determinado usuário está entre outros casos. Essas são algumas das situações em que conseguimos obter o resultado esperado através do geoprocessamento.
	
	\subsection{Dados não espaciais}
\indent \indent	
	Dados não espaciais, são também conhecidos por dados tabulares ou por atributos, descrevem características das entidades dos dados espaciais. Estas características podem ser quantitativas ou qualitativas. Por exemplo, as coordenadas de uma cidade são dados espaciais, enquanto que características dessa cidade como, numero de ruas, habitantes, casas, etc., seriam atributos.

	\section{Tecnologias de localização}
\indent \indent		
	Atualmente existem várias tecnologias capazes de obter localização que serão utilizadas por aplicações ou serviços que necessitem desta informação. Essas tecnologias são: GPS (\textit{Global positioning system}), AGPS (\textit{Assisted global positioning system}), \textit{Cell Identification} e \textit{Angle of Arrival}. O uso de cada uma dessas tecnologias dependerá principalmente do tipo de solução que se pretende adotar. Por esse motivo serão abordados somente as tecnologias GPS e AGPS já que atendem as necessidades desse projeto. A figura ~\ref{fig_tecnologiasLocalizacao} mostra as tecnologias GPS e AGPS.
	 	\begin{figure}[h]
		\begin{center}
			\includegraphics{Figuras/GPS_e_AGPS.png} 
			\caption{Tecnologias de Localização, GPS e AGPS.}
			\label{fig_tecnologiasLocalizacao}
		\end{center}
	\end{figure}
	
		\subsection{GPS}
\indent \indent	
O GPS foi criado pelo departamento de defesa dos Estados Unidos da America na década de 1970, com o objetivo inicial de ser usado somente para fins militares. Mas, a partir do ano de 2005 o acesso ao GPS foi liberado para a sociedade civil e hoje é um dos sistemas de posicionamento mais utilizado no mundo [9]. Segundo [6], o GPS é um sistema criado para navegação, utilizando sinais emitidos por satélites artificiais. Suas aplicações incluem navegação e posicionamento no mar, no ar e sobre a superfície terrestre.

Para o seu funcionamento foi necessário lançar ao espaço 28 satélites sendo 4 sobressalentes em 6 planos orbitais, ou seja, cada orbita possui 4 satélites que passam sobre o mesmo ponto da terra duas vezes por dia. A figura ~\ref{fig_satelitesOrbita} mostra os 28 satélites em suas orbitas. Desta forma é possível determinar a posição de um ponto qualquer que esteja em algum lugar da superfície da terra a qualquer hora do dia [10].

O GPS, ao receber o sinal do satélite determina o tempo exato que o sinal, contendo a distância e a localização, de no mínimo 4 satélites é possível determinar o tempo exato que o sinal levou durante todo o percurso. Com isso, é possível calcular a distância percorrida através da multiplicação desse tempo pela velocidade da luz. Essa distância é chamada de pseudodistância. Com essas informações é possível efetuar a triangulação das posições e assim determinar a localização do aparelho receptor.

No entanto, a precisão do GPS foi afetada propositadamente pelo departamento de defesa dos EUA introduzindo um erro na precisão de até 100 metros. No entanto, já existem técnicas para diminuir esse erro, chamadas de correção diferencial. Essa técnica utiliza uma estação base, contendo posições conhecidas, e algumas estações remotas. A correção diferencial é feita através da aplicação dos dados obtidos pela estação base, nos dados coletados das estações remotas. A depender da geometria dos satélites, a correção diferencial garanti uma precisão no posicionamento entre 1 a 10 metros.
Existem duas formas de realizar a correção diferencial:
	\begin{itemize}
	 		\item "	Correção diferencial em tempo real: Nesse método a estação base fica encarregada de fazer a correção dos erros gerados por cada satélite, e as envia imediatamente para os receptores que iram aplicar a correção no cálculo para gerar as posições [11];
 	 		\item "	Correção diferencial pós-processada: Nesse método a estação base e a estação remota armazenam os erros e suas posições, respectivamente, ao mesmo tempo. Esses dados serão guardados para servirem posteriormente no processamento e correção das posições dos receptores [11].		
	 		\end{itemize}
	 		
\begin{figure}[h]
		\begin{center}
			\includegraphics{Figuras/GPS_posicionamentoGlobal.png} 
			\caption{Satélites em orbita.}
			\label{fig_satelitesOrbita}
		\end{center}
	\end{figure}
	
	\subsection{AGPS}
\indent \indent	
A tecnologia AGPS esta presenta basicamente em smartphone que possuem também a tecnologia GPRS, e representa um aprimoramento do GPS, pois permite obter a localização do receptor em menos tempo e com pouco processamento. Com isso a bateria do dispositivo dura mais. 

Quando ligamos o GPS, o mesmo busca satélites para auxiliá-lo na navegação. Esse processo costuma demorar alguns minutos já que a conexão entre o GPS e o satélite não é rápida. No entanto, o AGPS se comunica primeiro com uma antena de telefonia celular que possui a localização dos outros satélites armazenada. E ainda o APGS consegue obter coordenadas mesmo estando em locais fechados, onde o sinal do GPS é fraco e em regiões com prédios altos. Mas se estiver em um local em que não há cobertura de celular, o aparelho funcionará como um GPS normal.
...
\section{API Google para Android}
\label{android:API}
\indent \indent
Uma das melhores maneiras de representar locais ou endereço é através de mapas. Para isso o Google possui uma API que fornece aos seus aplicativos Android fácil acesso aos serviços e dados do Google. Onde o recurso central é a biblioteca externa Maps que permite a edição de recursos avançados de mapeamento ao seu aplicativo Android. Com a biblioteca Maps é possível, por exemplo, efetuar marcações geográficas, obter coordenadas geográficas em tempo real ou sobre alguma área específica e a interação com os elementos que compõem o mapa. O pacote com.google.android.maps contém as classes MapView, MapController e MapActivity que são essenciais para trabalhar com o Google Maps. Veremos a seguir alguns desses componentes [13].
	
API (Application Programming Interface) é um conjunto de rotinas e padrões estabelecidos por um software para a utilização das suas funcionalidades por aplicativos que não pretendem envolver-se em detalhes da implementação do software, mas apenas usar seus serviços [14].

	\subsection{Classe MapView}
	\label{android:API:mapView}
	\indent \indent
	A classe MapView, do pacote com.google.android.maps.MapActivity, provê uma interface rica, própria para representar dados geográficos, pois encapsula todo o código necessário para realizar a comunicação com o GoogleMaps. Por este motivo é necessário informar a chave de acesso como parâmetro em seu construtor, além disso é preciso que a classe que representa o mapa estenda de MapActivity pois ela é a base para criar uma nova Activity que poderá incluir um mapa na aplicação. Através da classe MapView é possível obter total controle sobre o mapa podendo-lhe controlar o zoom, os tipos de visualização (satellite, street e traffic) e navegação. Veremos a seguir como obter a chave de acesso.
	A tabela \ref{tab:metosoMapview} descreve alguns dos métodos fornecidos pela classe MapView.
\begin{table}[htb!]
\begin{center}
{\footnotesize
...
Método  & Descrição \\ 
\hline 
setSatellite(boolean arg) & Habilita a visualização de satélite no mapa permitindo que o mapa seja preenchido com imagens de satélite. Caso esteja desabilitado o mapa representa a área visualizada através de desenho, utilizando padrão Google.\\ 
\hline 
getController() & Retorna um objeto MapController, o qual permite executar controle de zoom e navegação no mapa. \\ 
\hline 
setBuiltInZoomControls(boolean arg)  & Exibe o controle do zoom no mapa.\\ 
\hline 
getOverlays() & Retorna a lista com todos os elementos que irão compor o mapa. Todos os elementos que forem adicionados a essa lista serão exibidos no mapa. \\ 
\hline 
invalidate() & É usado para renderizar ou atualizar o mapa após a alteração ou inclusão de algum elemento na lista getOverlays(). \\ 
\hline 
\end{tabular} 
\end{center}
\caption{Tabela com alguns métodos da classe MapView.}
\label{tab:metosoMapview}
\end{table}

\subsection{Classe LocationManager}
	\label{android:API:locationManager}
	\indent \indent
	É o gerenciador de Localização usado para obter coordenadas geográficas. Através do mesmo podemos obter as coordenadas da localização atual, atualizações periódicas da localização do dispositivo e definir alertas para detectar quando estamos próximos de alguma área específica.
	
A classe LocationManager não pode ser instanciada diretamente, em vez disso, usamos Context.getSystemService(Context.LOCATION_SERVICE) para obter uma instância.

			
	\subsection{Classe GeoPoint}
	\label{android:API:geoPoint}
	\indent \indent
A classe GeoPoint representa um ponto geográfico ou uma localização, a qual é composta por um par de coordenadas geográficas (latitude e longitude), sendo representadas por microdegres. Microdegres é uma anotação que representa um grau multiplicado por 1E6 (1.000.000).
	
	\subsection{Classe MapController}
	\label{android:API:mapController}
	\indent \indent
	Essa classe disponibiliza recursos para controlar a navegação pelo mapa, além de permitir o controle sobre o zoom e criar animações no mapa.
Podemos ver na tabela \ref{tab:metodosMapController} alguns dos métodos utilizados nessa classe.
\begin{table}[htb!]
\begin{center}
...
Método  & Descrição \\ 
\hline 
setCenter(GeoPoint ponto) & Centraliza o mapa em um ponto específico. \\ 
\hline 
animateTo(GeoPoint ponto) & Usado para fazer animações, pois permite mover o mapa para um ponto específico. \\ 
\hline 
\end{tabular} 
\end{center}
\caption{Tabela com alguns métodos da classe MapController.}
\label{tab:metodosMapController}
\end{table}

	\subsection{Classe Overlay}
	\label{android:API:overlay}
	\indent \indent
Esta classe é utilizada quando queremos incluir um desenho no mapa. Veja na tabela \ref{tab:metodosOverlay} a descrição de alguns dos métodos fornecidos por esta classe.
...
Método  & Descrição \\ 
\hline 
Draw(Canvas canvas,MapView mapView,boolean shadow)  & Usado para definir a imagem que irá representar o elemento no mapa. \\ 
\hline 
\end{tabular}
\end{center}
\caption{Tabela com um dos métodos da classe Overlay.}
\label{tab:metodosOverlay}
\end{table}

\subsection{Obtendo \textit{API KEY} }
	\label{android:API:apiKey}
	\indent \indent
	A Google controla o acesso as funcionalidades do Google Maps, serviço que é acessado pelos aplicativos que utilizam mapas. Desta forma é necessário obter uma chave para utilizar esse serviço.
	
Como requisito para a solicitação da chave é necessário fornecer o código do certificado digital utilizado para assinar a aplicação desenvolvida. O desenvolvedor que utilizar o SDK e o plugin do Eclipse para desenvolvimento de aplicações Android terá automaticamente seu aplicativo assinado por um certificado digital no momento da compilação. Nesse caso a chave da assinatura será encontrado no certificado digital debug.keystore.

Para gerar a chave de acesso aos serviços do Google é necessário entrar no site \URL{https://developers.\linebreak google.com/maps/documentation/android/v1/maps-api-signup?hl=pt-BR}. Esse site possui um gerador da chave de certificação da Google, basta apenas informar a chave de assinatura do seu certificado digital e pressionar o botão Generate API Key. Feito isso é só colocar essa chave em sua aplicação. Veja na figura ~\ref{fig_Obtendo_API_KEY} explicando como fazer para gerar a chave de acesso.
		\begin{figure}[h]
		\begin{center}
		%[width=2cm]
			\includegraphics{Figuras/Obtendo_api_key.png} 
			\caption{Obtendo API Key.}
			\label{fig_Obtendo_API_KEY}
		\end{center}
	\end{figure}
...
\label{android:WBS}
\indent \indent
Web Service [1] é a tecnologia escolhida para ser o padrão de comunicação entre aplicações heterogêneas, não importando a tecnologia que ambas utiliza. Toda a comunicação tem como base o protocolo SOAP (Simple Object Access Protocol), onde a comunicação se dá através do envio e recebimento de dados em formato XML (Extensible Markup Language). Essa comunicação é feita entre Cliente e Servidor, onde o Servidor é aquele que possui os serviços e o cliente é aquele que consome os serviços. Na figura ~\ref{fig_Arquitetura_WebService} temos a arquitetura básica de um Web Service, onde podemos notar todo o seu potencial,  permitindo que máquinas com sistemas operacionais, limitações e fabricantes diferentes tenham acesso ao mesmo serviço. 
		\begin{figure}[h]
		\begin{center}
			\includegraphics{Figuras/Arquitetura_WebService.png} 
			\caption{Arquitetura do Web Service.}
			\label{fig_Arquitetura_WebService}
		\end{center}
	\end{figure}
O Web Service utiliza os protocolos XML (utilizada para descrever os dados trafegados) e HTTP (protocolo de troca de mensagens usado pela internet), como protocolo de comunicação entre sistemas diferentes. Porém esses dois protocolos definem apenas a maneira como ocorre a comunicação, por esse motivo surgiram padrões desenvolvidos por empresas como IBM e Microsoft com o intuito de tornar o uso de Web Services mais eficiente, dentre eles estão SOAP, WSDL e UDDI [15].

\begin{itemize}
	 \item WSDL (\textit{Web Service Description Language}) - Foi criado pela IBM no ano de 2000 após o surgimento do Web Service. WSDL trata-se de uma linguagem baseada em XML usada para descrever a interface do Web Service. As informações contidas em um arquivo WSDL se dividem em duas, naquelas que descrevem o Web Service de maneira mais abstrata e naquelas que descrevem o funcionamento do Web Service, como protocolo;
	 \item UDDI (\textit{Universal Description, Discovery and Innovation}) -  Este padrão também foi criado pela IBM no ano de 2000, é responsável pela localização do Web Service. O UDDI reúne as especificações sobre como as informações do Web Service devem ser vistas, acessadas ou modificas;
	 \item SOAP (\textit{Simple Object Access Protocol}) - Contém informações sobre o uso do Web Service, como, por exemplo, o formato da transferência de dados e informações sobre a requisição e resposta.
\end{itemize}
O sistema operacional Android não tem suporte nativo para o consumo de serviços web, por esse motivo foi utilizado a biblioteca kSOAP2[3] para a criação do Web Service. Qualquer outra biblioteca poderia ser utilizada, mas como estamos lidando com um dispositivo com recursos limitados de memória e processamento, escolhemos a kSOAP2[3] por ser a mais leve e por permite que aplicativos Android consumam serviços web de forma fácil e eficiente.

KSOAP é uma biblioteca baseada no protocolo SOAP que permite  que dispositivos limitados possa acessar serviços disponíveis por um Web Service. Através do KSOAP é possível criar uma aplicação cliente para acessar os serviços oferecidos por um Web Service. Mas o uso do KSOAP é restrito ao desenvolvimento de aplicações na plataforma Java, como applets ou aplicativos J2ME [16].

	
\section{Estrutura das Aplicações Android}
...
\label{aplicacao}
	...
	\label{aplicacao:requisitos}
	\indent \indent
	
	\section{Use Cases}
	\label{aplicacao:Use Case}
	\indent \indent

	\section{Arquitetura}
	\label{aplicacao:arquitetura}
	\indent \indent
	
	\section{Diagrama de Classe}
	\label{aplicacao:DiagramaClasse}
	\indent \indent

	\section{Diagrama de Entidade e Relacionamento}
	\label{aplicacao:DiagramaEntidadeRelacionamento}
	\indent \indent
	
	\section{Estudo de caso}
	\label{aplicacao:estudoCaso}
	\indent \indent
