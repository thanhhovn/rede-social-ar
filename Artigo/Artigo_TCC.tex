...
É fácil perceber, através da imagem \ref{fig_cicloVidaAtivity}, quais são as fases do ciclo onde o SO pode liberar recursos, ou seja, finalizar alguma \textit{Activity} que está muito tempo sem ser executada. E que também, no ciclo da \textit{Activity}, os únicos métodos que não são executados mais de uma vez são os métodos onCreat e o onDestroy.
	...
	O componente EditText, é uma expecialização do componente TextView. E representa um campo texto, onde o usuário pode digitar. Esse componente é referenciado no arquivo pela tag \flq EditText\frq. Veja um exemplo no código \ref{EditText}.
