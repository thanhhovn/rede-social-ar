...
O Android possui uma máquina virtual própria, chamada de \textit{Dalvik} (DVM), baseada na JVM (\textit{Java Virtual Machine}), mas, otimizada para consumir pouca memória e poder usar mais de uma instância simultaneamente sem prejudicar o desempenho do dispositivo, no qual cada aplicação roda em um processo próprio e restrito a somente está aplicação, ou seja, em uma instância da DVM. A DVM não opera bytecodes Java. Antes de serem executados na DVM, aplicativos escritos na linguagem Java e compilados por um compilador Java precisam ser transformados no formato binário .dex. Essa transformação é feita pela ferramenta "dx" incluída no SDK. O arquivo .dex é uma versão melhorada do .class (bytecode java) para dispositivos móveis. Após a conversão, o arquivo é então comprimido em um arquivo com extensão .apk.
...
Segundo Ricardo R. Lecheta autor do livro, "Google Android Aprenda a criar aplicações para dispositivos móveis com o Android SDK" \cite{Lecheta2010}, a \textit{Intent} pode ser utilizada para:

\begin{itemize}
	 \item Enviar um amensagem para osistema operacional;
